...
module cardinal_nic (
    ...
    // ----------------------------------------------------------
    // Synthesis-visible bypass outputs (for cardinal_node wrapper)
    // These expose internal state so the wrapper can build a
    // combinational read bypass without hierarchical references,
    // which are illegal for synthesis.
    // ----------------------------------------------------------
    output wire [63:0] bypass_input_buf,    // = net_input_buf
    output wire        bypass_input_status, // = input_status
    output wire        bypass_output_status // = output_status
);
    ...
    // Bypass outputs -- wire internal registers to top-level ports
    // so cardinal_node can build a combinational read mux without
    // using hierarchical references (which DC cannot synthesize).
    assign bypass_input_buf    = net_input_buf;
    assign bypass_input_status = input_status;
    assign bypass_output_status = output_status;
