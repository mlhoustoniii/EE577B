...
// Description  : EX/MEM Stage — ALU + memory interface (combinational)
//
// Boundary contract:
...
//     combinational pass-throughs gated on mem_en / mem_wr.
...
module ex_mem_stage (
    ...
    input          mem_en,      // DMEM access enable
    input          mem_wr,      // DMEM write enable (store)
    input  [0:15]  imm_addr,    // Immediate memory address

    // ---- Outputs ----
    output [0:63]  alu_result,  // Combinational ALU output
    output         memEn,       // DMEM enable (to top-level port)
    output         memWrEn,     // DMEM write enable (to top-level port)
...
always @(*) begin
    ...
    if (is_rtype) begin
        case (alu_func)
            ...
            // ww=00: bytes  0,2,4,6 → halfwords
            // ww=01: hwords 0,2     → words
            // ww=10: word   0       → doubleword
            // ww=11: not defined (ISA table stops at 32-bit input)
            // --------------------------------------------------
            FUNC_VMULEU: begin
                case (ww)
                    2'b00: begin
            ...
            // ww=00: bytes  1,3,5,7 → halfwords
            // ww=01: hwords 1,3     → words
            // ww=10: word   1       → doubleword
            // --------------------------------------------------
            FUNC_VMULOU: begin
                case (ww)
                    2'b00: begin
            ...
            // --------------------------------------------------
            // VSLL — shift left logical, shift amount from rB LSBs
            // ww=00: 3-bit shift amount per byte lane
            // ww=01: 4-bit shift amount per halfword lane
            // ww=10: 5-bit shift amount per word lane
            // ww=11: 6-bit shift amount for full doubleword
            // --------------------------------------------------
            FUNC_VSLL: begin
                case (ww)
                    2'b00: begin
            ...
            // --------------------------------------------------
            // VSRL — shift right logical, zeros fill MSBs
            // --------------------------------------------------
            FUNC_VSRL: begin
                case (ww)
                    2'b00: begin
            ...
            // --------------------------------------------------
            // VSRA — shift right arithmetic, sign bit fills MSBs
            // --------------------------------------------------
            FUNC_VSRA: begin
                case (ww)
                    2'b00: begin
            ...
            // --------------------------------------------------
            // VRTTH — rotate by half: swap upper/lower halves
            // ww=00: swap nibbles within each byte
            // ww=01: swap bytes within each halfword
            // ww=10: swap halfwords within each word
            // ww=11: swap words within doubleword
            // --------------------------------------------------
            FUNC_VRTTH: begin
                case (ww)
                    2'b00: begin
            ...
            // --------------------------------------------------
            // VDIV — unsigned integer divide; divide-by-zero → all 1s
            // --------------------------------------------------
            FUNC_VDIV: begin
                case (ww)
                    2'b00: begin
            ...
            // --------------------------------------------------
            // VMOD — unsigned integer modulo; mod-by-zero → dividend
            // --------------------------------------------------
            FUNC_VMOD: begin
                case (ww)
                    2'b00: begin
            ...
            // --------------------------------------------------
            // VSQEU — square even subfields, double-width result
            // ww=00: even bytes  (0,2,4,6) → halfwords
            // ww=01: even hwords (0,2)     → words
            // ww=10: even word   (0)       → doubleword
            // ww=11: not defined
            // --------------------------------------------------
            FUNC_VSQEU: begin
                case (ww)
            ...
            // --------------------------------------------------
            // VSQOU — square odd subfields, double-width result
            // ww=00: odd bytes  (1,3,5,7) → halfwords
            // ww=01: odd hwords (1,3)     → words
            // ww=10: odd word   (1)       → doubleword
            // --------------------------------------------------
            FUNC_VSQOU: begin
                case (ww)
                    2'b00: begin
            ...
            // --------------------------------------------------
            // VSQRT — unsigned integer square root, result same width
            // --------------------------------------------------
            FUNC_VSQRT: begin
                case (ww)
                    2'b00: begin
...
// ============================================================
//  Memory interface — purely combinational pass-through
// ============================================================
assign memEn    = mem_en;
assign memWrEn  = mem_wr;
assign addr_out = {16'b0, imm_addr};
assign d_out    = rA_data;   // store source: top level put forwarded rD here
