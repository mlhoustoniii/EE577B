`timescale 1ns/1ps
// ============================================================
// tb_gold_mesh_xystd.v -- Cardinal 4x4 Mesh Testbench (Standard XY variant)
...
module tb_gold_mesh_xystd;
    ...
    // All declared as per gold_mesh_xystd.v port list
    // ----------------------------------------------------------
    ...
    // ----------------------------------------------------------
    // xystd Packet Header Format (64-bit):
    ...
    initial begin
        ...
        // ---- Open output files ----
        fh_phase[0]  = $fopen("gather_phase_xystd_0.res",  "w");
        fh_phase[1]  = $fopen("gather_phase_xystd_1.res",  "w");
        fh_phase[2]  = $fopen("gather_phase_xystd_2.res",  "w");
        fh_phase[3]  = $fopen("gather_phase_xystd_3.res",  "w");
        fh_phase[4]  = $fopen("gather_phase_xystd_4.res",  "w");
        fh_phase[5]  = $fopen("gather_phase_xystd_5.res",  "w");
        fh_phase[6]  = $fopen("gather_phase_xystd_6.res",  "w");
        fh_phase[7]  = $fopen("gather_phase_xystd_7.res",  "w");
        fh_phase[8]  = $fopen("gather_phase_xystd_8.res",  "w");
        fh_phase[9]  = $fopen("gather_phase_xystd_9.res",  "w");
        fh_phase[10] = $fopen("gather_phase_xystd_10.res", "w");
        fh_phase[11] = $fopen("gather_phase_xystd_11.res", "w");
        fh_phase[12] = $fopen("gather_phase_xystd_12.res", "w");
        fh_phase[13] = $fopen("gather_phase_xystd_13.res", "w");
        fh_phase[14] = $fopen("gather_phase_xystd_14.res", "w");
        fh_phase[15] = $fopen("gather_phase_xystd_15.res", "w");
        fh_time      = $fopen("start_end_time_xystd.out", "w");

        // File headers
        for (i = 0; i < NUM_NODES; i = i + 1)
            $fdisplay(fh_phase[i],
                "// xystd Mesh Gather Results -- Dest node(%0d,%0d) id=%0d",

                NODE_X[i], NODE_Y[i], i);
        $fdisplay(fh_time, "// xystd 4x4 Mesh -- Phase Timing");
        $fdisplay(fh_time, "// ===================================");
        $fdisplay(fh_time, "// Sim start: %0t", $time);

        // ---- Waveform ----
        $dumpfile("tb_gold_mesh_xystd.vcd");
        $dumpvars(0, tb_gold_mesh_xystd);
