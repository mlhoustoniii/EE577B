...
// ============================================================
// gold_mesh_torus.v
// Cardinal Bidirectional 4x4 TORUS Network-on-Chip
//
...
//
// Routing: Torus XY modular-distance (min-hop)
//   dx_mod=(dst-src) mod 4: if mod==3, wrap (1 hop) beats forward (3 hops)
//
// Handshaking:
//   Transfer occurs when both si and ri are simultaneously high
// ============================================================

module gold_mesh_torus (
    input  wire        clk,
    input  wire        reset,
    ...
    // ==========================================================
    // Torus Wrap-Around Link Wires
    // Horizontal: (x=3,y).East <-> (x=0,y).West  for y=0..3
    // Vertical:   (x,y=3).North <-> (x,y=0).South for x=0..3
    // ==========================================================

    // Horizontal wrap wires (4 rows x 2 directions)
    wire        so_e_x3y0; wire ro_e_x3y0; wire [63:0] do_e_x3y0;
    wire        so_w_x0y0; wire ro_w_x0y0; wire [63:0] do_w_x0y0;
    wire        so_e_x3y1; wire ro_e_x3y1; wire [63:0] do_e_x3y1;
    wire        so_w_x0y1; wire ro_w_x0y1; wire [63:0] do_w_x0y1;
    wire        so_e_x3y2; wire ro_e_x3y2; wire [63:0] do_e_x3y2;
    wire        so_w_x0y2; wire ro_w_x0y2; wire [63:0] do_w_x0y2;
    wire        so_e_x3y3; wire ro_e_x3y3; wire [63:0] do_e_x3y3;
    wire        so_w_x0y3; wire ro_w_x0y3; wire [63:0] do_w_x0y3;

    // Vertical wrap wires (4 columns x 2 directions)
    wire        so_n_x0y3; wire ro_n_x0y3; wire [63:0] do_n_x0y3;
    wire        so_s_x0y0; wire ro_s_x0y0; wire [63:0] do_s_x0y0;
    wire        so_n_x1y3; wire ro_n_x1y3; wire [63:0] do_n_x1y3;
    wire        so_s_x1y0; wire ro_s_x1y0; wire [63:0] do_s_x1y0;
    wire        so_n_x2y3; wire ro_n_x2y3; wire [63:0] do_n_x2y3;
    wire        so_s_x2y0; wire ro_s_x2y0; wire [63:0] do_s_x2y0;
    wire        so_n_x3y3; wire ro_n_x3y3; wire [63:0] do_n_x3y3;
    wire        so_s_x3y0; wire ro_s_x3y0; wire [63:0] do_s_x3y0;
    ...
    gold_router_torus #(.ROUTER_X(8'd0), .ROUTER_Y(8'd0)) router_x0y0 (
        .clk      (clk),
        .reset    (reset),
        ...
        // South -- TORUS WRAP to (x=0, y=3)
        .ssi      (so_n_x0y3),  .sri  (ro_n_x0y3),
        .sdi      (do_n_x0y3),  .sso  (so_s_x0y0),
        .sro      (ro_s_x0y0),  .sdo  (do_s_x0y0),
        // East ? (1,0) west
        .esi      (so_w_x1y0),   .eri  (ro_w_x1y0),
        .edi      (do_w_x1y0),   .eso  (so_e_x0y0),
        .ero      (ro_e_x0y0),   .edo  (do_e_x0y0),
        // West -- TORUS WRAP to (x=3, y=0)
        .wsi      (so_e_x3y0),  .wri  (ro_e_x3y0),
        .wdi      (do_e_x3y0),  .wso  (so_w_x0y0),
        .wro      (ro_w_x0y0),  .wdo  (do_w_x0y0),
        // PE
    ...
    // South: TIED OFF
    gold_router_torus #(.ROUTER_X(8'd1), .ROUTER_Y(8'd0)) router_x1y0 (
        .clk      (clk),
        .reset    (reset),
        ...
        // South -- TORUS WRAP to (x=1, y=3)
        .ssi      (so_n_x1y3),  .sri  (ro_n_x1y3),
        .sdi      (do_n_x1y3),  .sso  (so_s_x1y0),
        .sro      (ro_s_x1y0),  .sdo  (do_s_x1y0),
        // East ? (2,0) west
    ...
    // South: TIED OFF
    gold_router_torus #(.ROUTER_X(8'd2), .ROUTER_Y(8'd0)) router_x2y0 (
        .clk      (clk),
        .reset    (reset),
        ...
        // South -- TORUS WRAP to (x=2, y=3)
        .ssi      (so_n_x2y3),  .sri  (ro_n_x2y3),
        .sdi      (do_n_x2y3),  .sso  (so_s_x2y0),
        .sro      (ro_s_x2y0),  .sdo  (do_s_x2y0),
        // East ? (3,0) west
    ...
    gold_router_torus #(.ROUTER_X(8'd3), .ROUTER_Y(8'd0)) router_x3y0 (
        .clk      (clk),
        .reset    (reset),
        ...
        // South -- TORUS WRAP to (x=3, y=3)
        .ssi      (so_n_x3y3),  .sri  (ro_n_x3y3),
        .sdi      (do_n_x3y3),  .sso  (so_s_x3y0),
        .sro      (ro_s_x3y0),  .sdo  (do_s_x3y0),
        // East -- TORUS WRAP to (x=0, y=0)
        .esi      (so_w_x0y0),  .eri  (ro_w_x0y0),
        .edi      (do_w_x0y0),  .eso  (so_e_x3y0),
        .ero      (ro_e_x3y0),  .edo  (do_e_x3y0),
        // West ? (2,0) east
    ...
    // West: TIED OFF
    gold_router_torus #(.ROUTER_X(8'd0), .ROUTER_Y(8'd1)) router_x0y1 (
        .clk      (clk),
        .reset    (reset),
        ...
        // West -- TORUS WRAP to (x=3, y=1)
        .wsi      (so_e_x3y1),  .wri  (ro_e_x3y1),
        .wdi      (do_e_x3y1),  .wso  (so_w_x0y1),
        .wro      (ro_w_x0y1),  .wdo  (do_w_x0y1),
        // PE
    ...
    gold_router_torus #(.ROUTER_X(8'd1), .ROUTER_Y(8'd1)) router_x1y1 (
        .clk      (clk),
        .reset    (reset),
    ...
    gold_router_torus #(.ROUTER_X(8'd2), .ROUTER_Y(8'd1)) router_x2y1 (
        .clk      (clk),
        .reset    (reset),
    ...
    // East: TIED OFF
    gold_router_torus #(.ROUTER_X(8'd3), .ROUTER_Y(8'd1)) router_x3y1 (
        .clk      (clk),
        .reset    (reset),
        ...
        // East -- TORUS WRAP to (x=0, y=1)
        .esi      (so_w_x0y1),  .eri  (ro_w_x0y1),
        .edi      (do_w_x0y1),  .eso  (so_e_x3y1),
        .ero      (ro_e_x3y1),  .edo  (do_e_x3y1),
        // West ? (2,1) east
    ...
    // West: TIED OFF
    gold_router_torus #(.ROUTER_X(8'd0), .ROUTER_Y(8'd2)) router_x0y2 (
        .clk      (clk),
        .reset    (reset),
        ...
        // West -- TORUS WRAP to (x=3, y=2)
        .wsi      (so_e_x3y2),  .wri  (ro_e_x3y2),
        .wdi      (do_e_x3y2),  .wso  (so_w_x0y2),
        .wro      (ro_w_x0y2),  .wdo  (do_w_x0y2),
        // PE
    ...
    gold_router_torus #(.ROUTER_X(8'd1), .ROUTER_Y(8'd2)) router_x1y2 (
        .clk      (clk),
        .reset    (reset),
    ...
    gold_router_torus #(.ROUTER_X(8'd2), .ROUTER_Y(8'd2)) router_x2y2 (
        .clk      (clk),
        .reset    (reset),
    ...
    // East: TIED OFF
    gold_router_torus #(.ROUTER_X(8'd3), .ROUTER_Y(8'd2)) router_x3y2 (
        .clk      (clk),
        .reset    (reset),
        ...
        // East -- TORUS WRAP to (x=0, y=2)
        .esi      (so_w_x0y2),  .eri  (ro_w_x0y2),
        .edi      (do_w_x0y2),  .eso  (so_e_x3y2),
        .ero      (ro_e_x3y2),  .edo  (do_e_x3y2),
        // West ? (2,2) east
    ...
    gold_router_torus #(.ROUTER_X(8'd0), .ROUTER_Y(8'd3)) router_x0y3 (
        .clk      (clk),
        .reset    (reset),
        .polarity (node03_polarity),
        // North -- TORUS WRAP to (x=0, y=0)
        .nsi      (so_s_x0y0),  .nri  (ro_s_x0y0),
        .ndi      (do_s_x0y0),  .nso  (so_n_x0y3),
        .nro      (ro_n_x0y3),  .ndo  (do_n_x0y3),
        // South ? (0,2) north
        ...
        // West -- TORUS WRAP to (x=3, y=3)
        .wsi      (so_e_x3y3),  .wri  (ro_e_x3y3),
        .wdi      (do_e_x3y3),  .wso  (so_w_x0y3),
        .wro      (ro_w_x0y3),  .wdo  (do_w_x0y3),
        // PE
    ...
    // North: TIED OFF
    gold_router_torus #(.ROUTER_X(8'd1), .ROUTER_Y(8'd3)) router_x1y3 (
        .clk      (clk),
        .reset    (reset),
        .polarity (node13_polarity),
        // North -- TORUS WRAP to (x=1, y=0)
        .nsi      (so_s_x1y0),  .nri  (ro_s_x1y0),
        .ndi      (do_s_x1y0),  .nso  (so_n_x1y3),
        .nro      (ro_n_x1y3),  .ndo  (do_n_x1y3),
        // South ? (1,2) north
    ...
    // North: TIED OFF
    gold_router_torus #(.ROUTER_X(8'd2), .ROUTER_Y(8'd3)) router_x2y3 (
        .clk      (clk),
        .reset    (reset),
        .polarity (node23_polarity),
        // North -- TORUS WRAP to (x=2, y=0)
        .nsi      (so_s_x2y0),  .nri  (ro_s_x2y0),
        .ndi      (do_s_x2y0),  .nso  (so_n_x2y3),
        .nro      (ro_n_x2y3),  .ndo  (do_n_x2y3),
        // South ? (2,2) north
    ...
    gold_router_torus #(.ROUTER_X(8'd3), .ROUTER_Y(8'd3)) router_x3y3 (
        .clk      (clk),
        .reset    (reset),
        .polarity (node33_polarity),
        // North -- TORUS WRAP to (x=3, y=0)
        .nsi      (so_s_x3y0),  .nri  (ro_s_x3y0),
        .ndi      (do_s_x3y0),  .nso  (so_n_x3y3),
        .nro      (ro_n_x3y3),  .ndo  (do_n_x3y3),
        // South ? (3,2) north
        .ssi      (so_n_x3y2),   .sri  (ro_n_x3y2),
        .sdi      (do_n_x3y2),   .sso  (so_s_x3y3),
        .sro      (ro_s_x3y3),   .sdo  (do_s_x3y3),
        // East -- TORUS WRAP to (x=0, y=3)
        .esi      (so_w_x0y3),  .eri  (ro_w_x0y3),
        .edi      (do_w_x0y3),  .eso  (so_e_x3y3),
        .ero      (ro_e_x3y3),  .edo  (do_e_x3y3),
        // West ? (2,3) east
