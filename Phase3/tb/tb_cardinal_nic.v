...
module tb_cardinal_nic;
    ...
    task check;
        ...
        begin
            if (actual === expected) begin
                $display("  [%0.2f ns] PASS [TC%0d] %s = %0b",
                         $realtime, tc, label, actual);
                pass_count = pass_count + 1;
            end else begin
                $display("  [%0.2f ns] FAIL [TC%0d] %s : got %0b expected %0b",
                         $realtime, tc, label, actual, expected);
    ...
    task check64;
        ...
        begin
            if (actual === expected) begin
                $display("  [%0.2f ns] PASS [TC%0d] %s = %h",
                         $realtime, tc, label, actual);
                pass_count = pass_count + 1;
            end else begin
                $display("  [%0.2f ns] FAIL [TC%0d] %s : got %h expected %h",
                         $realtime, tc, label, actual, expected);
                fail_count = fail_count + 1;
            end
        end
    ...
    reg [63:0] result;
    reg        accepted;
    // Distinct packets for preservation/overwrite checks.
    // Within each VC class, _a and _b differ in every nibble so a
    // bit-flip or silent overwrite in the buffer is immediately visible.
    // vc=0 (bit[63]=0) -- fire on net_polarity=0
    reg [63:0] pkt_vc0_a;   // original / first write
    reg [63:0] pkt_vc0_b;   // attempted overwrite (distinct from _a)
    // vc=1 (bit[63]=1) -- fire on net_polarity=1
    reg [63:0] pkt_vc1_a;
    reg [63:0] pkt_vc1_b;
    // Generic packets for input channel tests (vc bit does not matter
    // on the router->NIC path -- polarity only affects NIC->router)
    reg [63:0] pkt_a, pkt_b, pkt_c;
    integer    wcnt;
    ...
    initial begin
        ...
        // ------ Define test packets ------
        // Upper nibbles use 0x1111 / 0x2222 as visual markers so the
        // "original" vs "overwrite attempt" can be spotted in waves
        // at a glance. Lower 32 bits are distinct recognizable words.
        //
        // vc=0: bit[63]=0 -> net_so fires when net_polarity=0
        pkt_vc0_a = 64'h0000_1111_DEAD_BEEF;
        pkt_vc0_b = 64'h0000_2222_AAAA_5555;
        // vc=1: bit[63]=1 -> net_so fires when net_polarity=1
        pkt_vc1_a = 64'h8000_1111_CAFE_F00D;
        pkt_vc1_b = 64'h8000_2222_BEEF_CAFE;
        // Generic (input channel)
        pkt_a     = 64'h1234_5678_ABCD_EF01;
        pkt_b     = 64'hFEDC_BA98_7654_3210;
        pkt_c     = 64'hA5A5_5A5A_F0F0_0F0F;
        ...
        // Action:       nicEn=1, nicWrEn=1, addr=2'b10, d_in=pkt_vc0_a
        // Expected:
        //   - output_status -> 1 (at next posedge)
        //   - net_do = pkt_vc0_a (combinational, immediate)
        ...
        // Write pkt_vc0_a (vc=0) to output buffer
        proc_store(pkt_vc0_a);
        ...
        check64(net_do, pkt_vc0_a, "net_do = pkt_vc0_a after store", 3);
        ...
        // pkt_vc0_a[63]=0, so need net_polarity=0 for net_so to fire
        ...
        // Action:       nicEn=1, nicWrEn=1, addr=2'b10, d_in=pkt_vc0_b
        // Expected:
        //   - Write is SILENTLY IGNORED
        //   - Original packet (pkt_vc0_a) still in buffer (no corruption)
        //   - output_status remains 1
        //
        // CRITICAL: pkt_vc0_a and pkt_vc0_b MUST be distinct values.
        // If the NIC incorrectly latches the second write, net_do will
        // change from 0x0000_1111_DEAD_BEEF to 0x0000_2222_AAAA_5555
        // and the check fails loudly.
        //
        // Both packets share vc=0 so the drain condition is unchanged.
        //
        ...
        check64(net_do, pkt_vc0_a, "net_do=pkt_vc0_a (original)", 4);

        // Now attempt to write a DIFFERENT packet while buffer is full
        $display("  Attempting second store of pkt_vc0_b while buffer full (should be ignored)...");
        proc_store(pkt_vc0_b); // This should be IGNORED -- distinct from pkt_vc0_a

        @(posedge clk); #0.5;

        // net_do must STILL be pkt_vc0_a -- pkt_vc0_b must NOT have overwritten it
        check64(net_do, pkt_vc0_a,
                "net_do still=pkt_vc0_a (pkt_vc0_b ignored, not overwritten)", 4);
        ...
        net_ro = 1'b1;
        @(posedge clk); #0.5;
        // net_polarity=0 matches pkt_vc0_a[63]=0, so it should send now
        ...
        //   Full:  d_out = 64'h1
        //
        // Uses pkt_vc0_a as the test packet so net_do is visibly
        // distinct from the zero state and any stray overwrites
        // from other TCs would be spotted.
        // ==========================================================
        ...
        // Confirm the stored packet is the one we expect
        check64(net_do, pkt_vc0_a, "net_do=pkt_vc0_a (confirms buffer contents)", 8);
        ...
        proc_store(pkt_vc1_a);       // pkt_vc1_a[63]=1
        @(posedge clk); #0.5;
        ...
        @(negedge clk);
        net_polarity= 1'b1;   // now matches pkt_vc1_a[63]=1
        #0.1;
        check(net_so, 1'b1,
        ...
        @(negedge clk); net_ro = 1'b0;
        proc_store(pkt_vc0_a);        // pkt_vc0_a[63]=0
        @(posedge clk); #0.5;
        ...
        @(posedge clk); #0.5;
        check64(net_do, pkt_vc0_a, "net_do=pkt_vc0_a (immediate)", 12);
        ...
        //
        // net_ro toggles to simulate a busy router. This test uses
        // pkt_vc1_a so the packet value is distinct from the vc=0
        // packets used in TC12/TC11, making it easy to confirm in
        // waves that the right packet is flowing.
        //
        //   1. Proc writes pkt_vc1_a (vc=1)
        ...
        //   5. Confirm net_do still holds pkt_vc1_a (no corruption)
        //   6. Set net_ro=1 + polarity=1 -- net_so fires immediately
        //   7. At next posedge: output_status clears
        // ==========================================================
        ...
        @(negedge clk);
        net_ro      = 1'b0;
        net_polarity= 1'b1;   // pkt_vc1_a fires on polarity=1

        proc_store(pkt_vc1_a);
        @(posedge clk); #0.5;

        // Step 2: confirm stalled
        check(net_so, 1'b0,
              "net_so=0 while net_ro=0 (blocked)", 13);
        check64(net_do, pkt_vc1_a,
                "net_do=pkt_vc1_a (data held in buffer)", 13);
        $display("  Packet stalled -- output_status=1, net_ro=0");

        // Step 3: hold for several cycles and confirm stall persists
        // AND the buffer contents don't drift
        repeat(5) begin
            @(posedge clk); #0.5;
            check(net_so, 1'b0,
                  "net_so=0 persists while net_ro=0", 13);
            check64(net_do, pkt_vc1_a,
                    "net_do=pkt_vc1_a (no drift during stall)", 13);
        end
        ...
        check64(net_do, pkt_vc1_a,
                "net_do=pkt_vc1_a at moment of send", 13);
        ...
        // Restore default polarity for subsequent TCs
        net_polarity = 1'b0;
        ...
        //
        // CRITICAL: pkt_a and pkt_b MUST be distinct values.
        // If the NIC incorrectly latches the second send, the proc
        // read will return pkt_b instead of pkt_a and the check will
        // fail loudly.
        //
        ...
        $display("  Router attempts send of pkt_b while net_ri=0 (should ignore)...");
        @(negedge clk);
        net_si = 1'b1;  // Router ignores net_ri (drives anyway)
        net_di = pkt_b; // This must be ignored by NIC (distinct from pkt_a)
        ...
        // sends pkt_c -- verifies correct re-arm of input channel
        // after each read. Uses three distinct packets (pkt_a, pkt_c)
        // so any mix-up between rounds is immediately visible.
        //
        //   Cycle 1: router sends pkt_a -> NIC full
        //   Cycle 2: proc reads pkt_a  -> NIC empty -> net_ri=1
        //   Cycle 3: router sends pkt_c -> NIC full
        //   Cycle 4: proc reads pkt_c  -> verify correct data
        // ==========================================================
        ...
        // Round 2 -- immediate, with a DIFFERENT packet so mix-up is visible
        router_send(pkt_c, accepted);
        check(accepted, 1'b1, "Round2: pkt_c send accepted", 16);
        @(posedge clk); #0.5;
        check(net_ri, 1'b0,
              "Round2: net_ri=0 (buffer holds pkt_c)", 16);

        proc_load(2'b00, result);
        check64(result, pkt_c, "Round2: proc reads pkt_c (not stale pkt_a)", 16);
        check(net_ri, 1'b1,
        ...
        //
        // Fill BOTH buffers (input and output) with DISTINCT packets,
        // then assert reset. Verify all state clears:
        //   - Both buffers zeroed
        ...
        //   - net_do = 0
        //
        // Uses pkt_vc0_b (output) and pkt_c (input) as distinct
        // non-zero values so the post-reset check64(net_do, 0) can
        // only pass if the buffer actually cleared -- not just
        // because it happened to contain zero already.
        // ==========================================================
        $display("=== TC17: Re-Reset Mid-Simulation ===");

        // Step 1: fill output buffer (block so it stays)
        @(negedge clk); net_ro = 1'b0;
        proc_store(pkt_vc0_b);
        @(posedge clk); #0.5;
        check64(net_do, pkt_vc0_b,
                "pre-TC17: net_do=pkt_vc0_b (confirms non-zero fill)", 17);
        proc_load(2'b11, result);
        check64(result, 64'h1,
                "pre-TC17: output_status=1 (buffer full)", 17);

        // Step 2: fill input buffer with a different distinct packet
        router_send(pkt_c, accepted);
        @(posedge clk); #0.5;
        proc_load(2'b01, result);
        check64(result, 64'h1,
        ...
        // Both buffers full with distinct non-zero packets -- apply reset
        $display("  Applying reset with both buffers full (out=pkt_vc0_b, in=pkt_c)...");
        @(negedge clk);
        reset       = 1'b1;
        net_ro      = 1'b1; // restore
        repeat(RST_CYCS) @(posedge clk); #0.5;

        // Check all reset conditions -- net_do=0 is a meaningful check
        // only because the buffer held a non-zero value before reset
        check(net_ri, 1'b1,
        ...
        check64(net_do, 64'h0,
                "net_do=0 after mid-sim reset (buffer cleared)", 17);
        ...
        //   Uses pkt_vc0_a (outbound) and pkt_b (inbound) -- distinct
        //   values so a cross-wired buffer bug would be visible.
        // ==========================================================
        ...
        // b. Proc stores pkt_vc0_a (vc=0, fires on polarity=0)
        $display("  Proc writes pkt_vc0_a to output buffer");
        proc_store(pkt_vc0_a);
        @(posedge clk); #0.5;
        ...
        // polarity=0 already set; net_ro=1; pkt_vc0_a[63]=0 -> match
        check(net_so, 1'b1,
              "Outbound: net_so=1 (polarity matches vc=0)", 18);
        check64(net_do, pkt_vc0_a,
                "Outbound: net_do=pkt_vc0_a on network", 18);
        ...
        // h. Router sends pkt_b (distinct from outbound pkt_vc0_a)
