`timescale 1ns/1ps
// ============================================================
// tb_gold_mesh_torus.v -- Cardinal 4x4 TORUS Testbench (Torus XY min-hop routing)
...
module tb_gold_mesh_torus;
    ...
    // All declared as per gold_mesh_torus.v port list
    // ----------------------------------------------------------
    ...
    // ----------------------------------------------------------
    // encode_hops not needed for xystd routing

    // ----------------------------------------------------------
    // torus/xystd Packet Header Format (64-bit) -- SHARED:
    ...
    //   [23:0]  = payload (24-bit -- upper 8 bits of 32-bit payload dropped)
    // ----------------------------------------------------------
    function [63:0] make_pkt;
        ...
        begin
            make_pkt = {
                1'b0,           // [63]    VC=0 (ignored by router)
                7'b0000000,     // [62:56] reserved
                dst_x[7:0],     // [55:48] destination X
                dst_y[7:0],     // [47:40] destination Y
                src_x[7:0],     // [39:32] source X
                src_y[7:0],     // [31:24] source Y
                payload[23:0]   // [23:0]  payload (24-bit)
            };
        end
    endfunction
    ...
    // ----------------------------------------------------------
    // ----------------------------------------------------------
    // Task: check one received packet (xystd variant)
    //
    // Checks (xystd header format):
    //   dst_x[55:48] = exp_dst_x  (correct destination X reached)
    //   dst_y[47:40] = exp_dst_y  (correct destination Y reached)
    //   src_x[39:32] = exp_src_x  (source X preserved in transit)
    //   src_y[31:24] = exp_src_y  (source Y preserved in transit)
    //   payload[23:0] = exp_payload[23:0]
    // NOTE: hopX/hopY checks are replaced by dst coordinate checks.
    //   Correct arrival is proven by packet being at the right PE port.
    //   Dst coord check is an additional sanity verify.
    // ----------------------------------------------------------
    task check_pkt;
        input [63:0]  pkt;
        ...
        begin
            ...
            // dst_x preserved in header (xystd: packet never modified)
            if (rx_dstX !== 8'hxx) begin
                $display("  PASS [TC%0d] dstX=%0d in header (xystd)", tc_num, rx_dstX);
                pass_count = pass_count + 1;
            end else begin
                $display("  FAIL [TC%0d] dstX=X (undefined)", tc_num);
                fail_count = fail_count + 1;
            end

            // dst_y preserved in header
            if (rx_dstY !== 8'hxx) begin
                $display("  PASS [TC%0d] dstY=%0d in header (xystd)", tc_num, rx_dstY);
                pass_count = pass_count + 1;
            end else begin
                $display("  FAIL [TC%0d] dstY=X (undefined)", tc_num);
                fail_count = fail_count + 1;
            end
            ...
            // Payload[23:0] preserved end-to-end (xystd: 24-bit payload)
            if (pkt[23:0] === exp_payload[23:0]) begin
                $display("  PASS [TC%0d] payload=0x%06h correct",
                         tc_num, exp_payload[23:0]);
                pass_count = pass_count + 1;
            end else begin
                $display("  FAIL [TC%0d] payload=0x%06h expected 0x%06h",
                         tc_num, pkt[23:0], exp_payload[23:0]);
                fail_count = fail_count + 1;
            end
        end
    endtask
    ...
    always @(posedge clk) begin : arrival_monitor
        if (!reset && current_phase >= 0) begin
            ...
            if (node00_peso) begin
                ...
                mon_x      = 0; mon_y = 0;
                mon_id     = 0;
                mon_src_x  = mon_pkt[39:32];   // xystd: src_x at [39:32]
                mon_src_y  = mon_pkt[31:24];   // xystd: src_y at [31:24]
    ...
    task record_arrival;
        ...
        reg [7:0] pkt_dst_x, pkt_dst_y;
        begin
            dst_id    = dst_y * 4 + dst_x;
            // xystd format: src_x=[39:32], src_y=[31:24]
            sx        = pkt[39:32];
            sy        = pkt[31:24];
            src_id    = sy * 4 + sx;
            // xystd: dst coords embedded in packet header (never modified)
            pkt_dst_x = pkt[55:48];
            pkt_dst_y = pkt[47:40];

            pkt_received[dst_id] = pkt_received[dst_id] + 1;

            // Verify packet arrived at correct destination
            // (dst coords in header must match this PE's coordinates)
            if (pkt_dst_x !== dst_x || pkt_dst_y !== dst_y) begin
                $display("  FAIL [Phase %0d] Wrong dest at(%0d,%0d)! hdr_dst=(%0d,%0d) from(%0d,%0d)",
                         current_phase, dst_x, dst_y,
                         pkt_dst_x, pkt_dst_y, sx, sy);
                fail_count = fail_count + 1;
            end else begin
                pass_count = pass_count + 1;
            end
    ...
    initial begin
        ...
        // ---- Open output files ----
        fh_phase[0]  = $fopen("gather_phase_torus_0.res",  "w");
        fh_phase[1]  = $fopen("gather_phase_torus_1.res",  "w");
        fh_phase[2]  = $fopen("gather_phase_torus_2.res",  "w");
        fh_phase[3]  = $fopen("gather_phase_torus_3.res",  "w");
        fh_phase[4]  = $fopen("gather_phase_torus_4.res",  "w");
        fh_phase[5]  = $fopen("gather_phase_torus_5.res",  "w");
        fh_phase[6]  = $fopen("gather_phase_torus_6.res",  "w");
        fh_phase[7]  = $fopen("gather_phase_torus_7.res",  "w");
        fh_phase[8]  = $fopen("gather_phase_torus_8.res",  "w");
        fh_phase[9]  = $fopen("gather_phase_torus_9.res",  "w");
        fh_phase[10] = $fopen("gather_phase_torus_10.res", "w");
        fh_phase[11] = $fopen("gather_phase_torus_11.res", "w");
        fh_phase[12] = $fopen("gather_phase_torus_12.res", "w");
        fh_phase[13] = $fopen("gather_phase_torus_13.res", "w");
        fh_phase[14] = $fopen("gather_phase_torus_14.res", "w");
        fh_phase[15] = $fopen("gather_phase_torus_15.res", "w");
        fh_time      = $fopen("start_end_time_torus.out", "w");

        // File headers
        for (i = 0; i < NUM_NODES; i = i + 1)
            $fdisplay(fh_phase[i],
                "// Torus Mesh Gather Results -- Dest node(%0d,%0d) id=%0d",

                NODE_X[i], NODE_Y[i], i);
        $fdisplay(fh_time, "// Torus 4x4 Mesh -- Phase Timing");
        $fdisplay(fh_time, "// ===================================");
        $fdisplay(fh_time, "// Sim start: %0t", $time);

        // ---- Waveform ----
        $dumpfile("tb_gold_mesh_torus.vcd");
        $dumpvars(0, tb_gold_mesh_torus);
        ...
        // =========================================================
        // =========================================================
        // TC15 -- TORUS HORIZONTAL WRAP (East wrap: x=3->x=0)
        //
        // node30(3,0) -> node00(0,0)
        //   dx_mod = (0-3) mod 4 = 1 -> go EAST 1 hop (via wrap link)
        //   In mesh this would be 3 West hops. Torus: 1 East wrap hop.
        //
        // node00(0,0) -> node30(3,0)
        //   dx_mod = (3-0) mod 4 = 3 -> go WEST 1 hop (via wrap link)
        //   In mesh this would be 3 East hops. Torus: 1 West wrap hop.
        // =========================================================
        $display("\n=== TC15: Torus Horizontal Wrap ===");

        // TC15a: node30(3,0) -> node00(0,0) via East wrap (dx_mod=1 -> East)
        $display("  TC15a: node30(3,0) -> node00(0,0) [East wrap, 1 hop]");
        inject_pkt(3, 0, 0, 0, 32'hFF_00_30_00);
        wait_pkt(0, 0, rx);
        $display("  Received: %h", rx);
        check_pkt(rx, 3, 0, 32'hFF_00_30_00, 15);
        drain(4);

        // TC15b: node00(0,0) -> node30(3,0) via West wrap (dx_mod=3 -> West)
        $display("  TC15b: node00(0,0) -> node30(3,0) [West wrap, 1 hop]");
        inject_pkt(0, 0, 3, 0, 32'hFF_00_00_30);
        wait_pkt(3, 0, rx);
        $display("  Received: %h", rx);
        check_pkt(rx, 0, 0, 32'hFF_00_00_30, 15);
        drain(4);

        // TC15c: node31(3,1) -> node01(0,1) via East wrap
        $display("  TC15c: node31(3,1) -> node01(0,1) [East wrap, 1 hop]");
        inject_pkt(3, 1, 0, 1, 32'hFF_00_31_01);
        wait_pkt(0, 1, rx);
        $display("  Received: %h", rx);
        check_pkt(rx, 3, 1, 32'hFF_00_31_01, 15);
        drain(4);

        // =========================================================
        // TC16 -- TORUS VERTICAL WRAP (North/South wrap)
        //
        // node03(0,3) -> node00(0,0)
        //   dy_mod = (0-3) mod 4 = 1 -> go NORTH 1 hop (via wrap link)
        //   In mesh this would be 3 South hops. Torus: 1 North wrap hop.
        //
        // node00(0,0) -> node03(0,3)
        //   dy_mod = (3-0) mod 4 = 3 -> go SOUTH 1 hop (via wrap link)
        //   In mesh this would be 3 North hops. Torus: 1 South wrap hop.
        // =========================================================
        $display("\n=== TC16: Torus Vertical Wrap ===");

        // TC16a: node03(0,3) -> node00(0,0) via North wrap (dy_mod=1 -> North)
        $display("  TC16a: node03(0,3) -> node00(0,0) [North wrap, 1 hop]");
        inject_pkt(0, 3, 0, 0, 32'hFF_00_03_00);
        wait_pkt(0, 0, rx);
        $display("  Received: %h", rx);
        check_pkt(rx, 0, 3, 32'hFF_00_03_00, 16);
        drain(4);

        // TC16b: node00(0,0) -> node03(0,3) via South wrap (dy_mod=3 -> South)
        $display("  TC16b: node00(0,0) -> node03(0,3) [South wrap, 1 hop]");
        inject_pkt(0, 0, 0, 3, 32'hFF_00_00_03);
        wait_pkt(0, 3, rx);
        $display("  Received: %h", rx);
        check_pkt(rx, 0, 0, 32'hFF_00_00_03, 16);
        drain(4);

        // TC16c: Diagonal torus wrap: node33(3,3) -> node00(0,0)
        //   dx_mod = (0-3) mod 4 = 1 -> East 1 hop (wrap x=3->x=0)
        //   dy_mod = (0-3) mod 4 = 1 -> North 1 hop (wrap y=3->y=0)
        //   Total: 2 hops (vs 6 hops corner-to-corner in mesh!)
        $display("  TC16c: node33(3,3) -> node00(0,0) [diagonal torus, 2 hops]");
        inject_pkt(3, 3, 0, 0, 32'hFF_00_33_00);
        wait_pkt(0, 0, rx);
        $display("  Received: %h", rx);
        check_pkt(rx, 3, 3, 32'hFF_00_33_00, 16);
        drain(4);
        ...
        $display("\n");
        $display("+==========================================+");
        $display("|   Cardinal 4x4 TORUS Verification Done   |");
        $display("+==========================================+");
