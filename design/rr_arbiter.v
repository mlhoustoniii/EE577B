...
// ============================================================
// rr_arbiter.v
// Cardinal Router -- Round-Robin Arbiter (4-way)
//
...
//      This is required for the 2-cycle latency target --
//      a registered grant would add an extra unwanted cycle.
//
//   2. Separate priority pointers per VC type (even/odd):
//      Per spec: "priority tracking must be maintained for each
//      virtual channel type." Even VC contention must not affect
//      odd VC priority order and vice versa.
//
//   3. Pointer only advances on contention:
//      Per spec: "If there is no contention, priority tracking
//      is unchanged." Only advance when 2+ inputs request.
//      Pointer advances to (winner+1) mod 4.
//
//   4. No for-loops -- fully unrolled per synthesis requirement.
//      (Slide 9: for-loops not synthesis-friendly.)
//
// Port Description:
//   polarity         -- 0=even cycle, 1=odd cycle (from router)
//   request[3:0]     -- one-hot request from 4 input slots
//   output_available -- output buffer free this cycle
//   grant[3:0]       -- one-hot grant (combinational Mealy output)
//
// Slot ordering is set by gold_router instantiation to match
...
module rr_arbiter (
    ...
    //
    // Contention = 2 or more inputs requesting simultaneously.
    // Pointer only advances when contention occurs.
    // Fully enumerated -- no for-loops per synthesis rule.
    // ----------------------------------------------------------
    wire contention;
    ...
    //
    // Walk priority chain starting at current_ptr.
    // First active request encountered in priority order wins.
    // No grant if output unavailable or no requests.
    // ----------------------------------------------------------
    reg [3:0] grant_comb;
    ...
    // Grant is purely combinational -- Mealy output (no register)
    assign grant = grant_comb;

    // ----------------------------------------------------------
    // Priority pointer update -- sequential (clocked)
    //
    // Rule: pointer advances ONLY when contention occurred AND
    //       output was available AND at least one request was active.
    //
    // Pointer advances to (winner + 1) mod 4, so winner's neighbor
    // has highest priority next round (fair rotation).
    //
    // polarity selects WHICH pointer to update:
    //   even cycle -> update even pointer
    //   odd  cycle -> update odd  pointer
    //   The other pointer is untouched.
    // ----------------------------------------------------------
