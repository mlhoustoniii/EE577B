`timescale 1ns/1ps
// tb_perf_hop.v -- HOP perf TB  DUT:gold_mesh
module tb_perf_hop;
    ...
    function [3:0] unh; input integer n; begin
        case(n) 0:unh=4'h0;1:unh=4'h1;2:unh=4'h3;default:unh=4'h7; endcase
    end endfunction
    function [63:0] make_pkt;
        input integer sx,sy,dx,dy; input [15:0] ts; input pol;
        reg signed [31:0] ddx,ddy; reg [3:0] hx,hy; reg dirx,diry; reg [7:0] did;
        begin
            ddx=dx-sx; ddy=dy-sy;
            dirx=(ddx<0)?1'b1:1'b0; diry=(ddy>0)?1'b1:1'b0;
            hx=unh((ddx<0)?-ddx:ddx); hy=unh((ddy<0)?-ddy:ddy);
            did=dy*4+dx;
            make_pkt={pol,dirx,diry,5'b0,hx,hy,sx[7:0],sy[7:0],8'h00,did,ts};
        end
    endfunction
    ...
    integer inj_cyc;
    initial begin
        $dumpfile("tb_perf_hop.vcd"); $dumpvars(0,tb_perf_hop);
        ...
        fh=$fopen("perf_hop_summary.txt","w");
        $fdisplay(fh,"======================================================");
        $fdisplay(fh,"  EE577B Cardinal Router -- HOP Performance Report");
        $fdisplay(fh,"  DUT: gold_mesh");
        $fdisplay(fh,"======================================================");
        ...
        $fdisplay(fh,"  Max freq : 258.4 MHz");
        $fdisplay(fh,"  Crit path: 3.820 ns  Slack: +0.130 ns");
        $fdisplay(fh,"  Cell area: 30135 um2  Power: 5.903 mW");
        $fdisplay(fh,"  Sim cycles: %%0d", cycle_count);
        $fdisplay(fh,"======================================================");
        $fclose(fh);
        $display("[HOP] DONE. %0d/36 pairs ok. Min=%0d Max=%0d Avg=%.1f cyc",
                cnt, mn, mx, avg);
        $finish;
    end
endmodule